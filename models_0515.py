...
def base_block2(start_nums, end_nums):
    block = [nn.Conv2d(start_nums, end_nums, kernel_size=3, stride=1, padding=1, bias=True), ]
    block += [nn.LeakyReLU(True), ]
    # block+=[nn.LeakyReLU(True),]
    block += [nn.Conv2d(end_nums, end_nums, kernel_size=3, stride=1, padding=1, bias=True), ]
    block += [nn.LeakyReLU(True), ]
    # block+=[nn.LeakyReLU(True),]
...
def res_block(start_nums, end_nums):
    mid_nums = int(end_nums / 4)
    if mid_nums < 1:
        mid_nums = 1
...
def res_block_up(start_nums, end_nums):
    mid_nums = int(end_nums / 4)
    if mid_nums < 1:
        mid_nums = 1
    block = [nn.Conv2d(start_nums, mid_nums, kernel_size=1, stride=1, padding=0, bias=True), ]
...
class myGenerator(BaseColor):
    def __init__(self, norm_layer=nn.BatchNorm2d, in_nums=3, out_nums=3, F_nums=[128, 256, 512, 512]):
        ...
        end_nums = 32  # 等同1出
        model9up = [nn.ConvTranspose2d(start_nums, end_nums, kernel_size=4, stride=2, padding=1, bias=True), ]
        model2short9 = [nn.Conv2d(start_nums, end_nums, kernel_size=3, stride=1, padding=1, bias=True), ]
        # add the two feature maps above
        ...
        start_nums = 32
        end_nums = 32
        model10up = [nn.ConvTranspose2d(start_nums, end_nums, kernel_size=4, stride=2, padding=1, bias=True), ]  # 图像2倍
        model1short10 = [nn.Conv2d(start_nums, end_nums, kernel_size=3, stride=1, padding=1, bias=True), ]  # 形状不变
        # add the two feature maps above

        model10 = [nn.ReLU(True), ]
        model10 += [nn.Conv2d(end_nums, end_nums, kernel_size=3, dilation=1, stride=1, padding=1, bias=True), ]
        model10 += [nn.LeakyReLU(negative_slope=.2), ]

        # regression output
        model_out = [nn.Conv2d(end_nums, end_nums, kernel_size=1, padding=0, dilation=1, stride=1, bias=True), ]
        model_out += [nn.Conv2d(end_nums, out_nums, kernel_size=1, padding=0, dilation=1, stride=1, bias=True), ]
        sigmoid = [nn.Sigmoid(), ]
        # model_out+=[nn.Tanh()]
        #############################################
        ...
        self.M_rout = nn.Sequential(*base_block(F_nums[0], out_nums))
...
class NLayerGenerator(BaseColor):
    def __init__(self, input_nums, output_nums, ndf=8, n_layers=8, norm_layer=nn.BatchNorm2d, use_bias=False,
                 use_sigmoid=False):
...
class NLayerDiscriminator(BaseColor):
    def __init__(self, input_nums, output_nums, ndf=16, n_layers=3, norm_layer=nn.BatchNorm2d, use_bias=False,
                 use_sigmoid=False):
